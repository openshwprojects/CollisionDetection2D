...
enum PlaneSide { PS_ON, PS_FRONT, PS_BACK };

enum PlaneRelation { PR_Parallel, PR_Coincident, PR_Intersecting };
...
public:
	...
	bool compare(const Plane2D& o, float epsD, float epsN) const {
		if (abs(o.distance - this->distance) > epsD) return false;
		if (normal.compare(o.normal, epsN) == false) return false;
		return true;
	}
	...
	PlaneLineRelation intersectLineExt(const Vec2D& segmentStart, const Vec2D& segmentEnd,
									   Vec2D* res) const {
		float distanceStart = this->distanceTo(segmentStart);
		float distanceEnd = this->distanceTo(segmentEnd);
		// on front of the plane
		if (distanceStart > 0 && distanceEnd > 0) return PLR_Front;
		// behind the plane
		if (distanceStart < 0 && distanceEnd < 0) return PLR_Behind;
		float eps = 0.001f;
		// on plane
	...
	bool intersectLine(const Vec2D& segmentStart, const Vec2D& segmentEnd, Vec2D* res) const {
		...
		float dotProduct =
			segmentToPlane.getX() * normal.getX() + segmentToPlane.getY() * normal.getY();
		float directionDotProduct =
			segmentDirection.getX() * normal.getX() + segmentDirection.getY() * normal.getY();
		...
		if (t >= 0.0f && t <= 1.0f) {
			...
			if (res) {
				*res = segmentStart + segmentDirection * t;
				...
				Vec2D verify;
				ASSERT_TRUTH(
					PLR_Intersects == this->intersectLineExt(segmentStart, segmentEnd, &verify),
					"Both plane vs line gives the same result");
				ASSERT_FLOAT_EQUALS(res->distanceTo(verify), 0.0f, 0.1f,
									"Both plane vs line gives the same result");
#endif
			}
			return true;
	...
	PlaneRelation intersectPlane(const Plane2D& other, Vec2D& res) const {
		...
		if (dotProduct == 1.0f) {
			...
			float determinant =
				normal.getX() * other.normal.getY() - normal.getY() * other.normal.getX();
			float x =
				(other.normal.getY() * -distance - normal.getY() * -other.distance) / determinant;
			float y =
				(normal.getX() * -other.distance - other.normal.getX() * -distance) / determinant;
			Vec2D intersectionPoint(x, y);

			// Check if the planes are coincident
			if (intersectionPoint.getX() * normal.getX() +
						intersectionPoint.getY() * normal.getY() ==
					distance &&
				intersectionPoint.getX() * other.normal.getX() +
						intersectionPoint.getY() * other.normal.getY() ==
					other.distance) {
	...
	PlaneSide getPointPlaneRelation(const Vec2D& pointVal, float* outDist = 0,
									float epsilon = 0.001f) const {
