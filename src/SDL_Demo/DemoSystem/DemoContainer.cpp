...
void CDemoContainer::drawDebugTexts() {
	...
	for (int i = 0; i < settings.size(); i++) {
		...
		if (s.name.size()) {
			...
			if (s.pfloat) {
				drawDebugText(" - %s = %f - press %c %c to change", s.name.c_str(), *s.pfloat,
							  s.keyU, s.keyD);
			} else {
...
bool CDemoContainer::isKeyInUse(int key) const {
	for (int i = 0; i < settings.size(); i++) {
		if (settings[i].key == key) return true;
	}
	return false;
}
...
void CDemoContainer::addSetting(float* value, float step, float min, float max, const char* name,
								int keyU, int keyD) {
